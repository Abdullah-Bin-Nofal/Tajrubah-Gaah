...
namespace Tajurbah_Gah
{
    public class StopwatchController : MonoBehaviour
    {
        ...
        {
            PauseStopWatch();
            stopwatchTimeText.text = "00:00:00";
            stopwatchTime = 0;
            Init();
        }
        ...
        void DisplayTime(float TimeToDisplay)
        {
            ...
            MillisecondsText.text = null;


            if (minutes)
            {
                stopwatchTimeText.text += string.Format("{0:00}", MinutesValue);
                MinutesText.text += string.Format("{0:00}", MinutesValue) + ":";
            }
            if (seconds)
            {
                stopwatchTimeText.text += string.Format(minutes?":{0:00}":"{0:00}", SecondsValue);
                SecondsText.text += string.Format("{0:00}", SecondsValue) + ":";
            }
            if (milliSeconds)
            {
                stopwatchTimeText.text += string.Format(seconds?":{00:00}": "{00:00}", Mathf.FloorToInt(MilliSecondsValue/10));
                MillisecondsText.text += string.Format("{0:00}", Mathf.FloorToInt(MilliSecondsValue / 10));
            }
            ...
            mytime = (MinutesValue * 60) + (SecondsValue) + (((MilliSecondsValue/10)) / 100);
